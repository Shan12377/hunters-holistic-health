...
| 9 | My Protocol redesign: My Day view | CODE DONE July 19, requested by Dr. Hunter. New default "My Day" tab groups every assigned item into Morning, With meals, Evening, and Anytime (parsed from the timing text she set), with tap-to-check circles that reset daily and a done counter. Summary chips under the title (N supplements, phases, "Prepared for you by Dr. Hunter"). Phase stepper, Overview, and Supplements tabs unchanged. Also fixed dashboard My Protocol quick action to point at /app/my-protocol instead of the ROOTS page. | MyProtocolPage.tsx, ProtocolPage.module.css, ClientDashboard.tsx |

| 10 | Push reminders wired end to end | CODE DONE July 19. Found: sending function (api/cron-reminders.ts), settings UI (Settings page), and subscription plumbing all existed but were never connected. Two missing links fixed: (a) new public/push-sw.js gives the service worker push and notification-click handlers via workbox importScripts, without which sent pushes never display; (b) vercel.json now has a daily cron (0 11 * * *, which is 7 AM Eastern in summer) calling /api/cron-reminders, sized for the Hobby plan which only allows daily crons. The function was reworked for daily mode: it sends one morning nudge ("Good morning from Hunter's Holistic Health") to every subscribed member with any reminder enabled, which also removes the UTC timezone mismatch since everyone gets it at the same Eastern hour. Upgrade path (comment in the function): on Vercel Pro, set the schedule back to */30 and restore per-time matching for the individual fasting, supplement, and afternoon reminders. Before this works: VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY, and CRON_SECRET must exist in Vercel env vars. | public/push-sw.js, vite.config.ts, vercel.json, api/cron-reminders.ts |

| 11 | Crash visibility + real 404 | CODE DONE July 19. New api/log-error.ts (public by design, rate limited, stores nothing) receives crash reports from the ErrorBoundary, so frontend crashes now appear in Vercel function logs under [client-error]. New NotFoundPage replaces the silent catch-all redirect to the homepage. To see crash reports: Vercel dashboard, the project, Logs, filter for client-error. | api/log-error.ts, ErrorBoundary.tsx, NotFoundPage.tsx, App.tsx |

| 12 | Final four: nudge, data export, a11y, page views | CODE DONE July 19. (a) Educator roster rows now have a gold Nudge button that sends a warm pre-filled check-in message straight to that participant's inbox, one tap, marks Sent. (b) Settings gained a Download My Data card exporting all the client's logs as a JSON file. (c) Accessibility: aria-labels added to the two unlabeled educator delete buttons; the rest of the app's icon-only controls were already labeled. (d) Page-view tracking: migration supabase/migrations/039_page_views.sql (Dr. Hunter must paste it into Supabase SQL Editor and run it) plus a fire-and-forget tracker in AppLayout; clients insert their own rows, only educators can read. Until the migration runs, inserts silently fail and nothing breaks. | CoachDashboard.tsx, Coach.module.css, SettingsPage.tsx, ManageChallengesPage.tsx, ManageClassroomPage.tsx, AppLayout.tsx, 039_page_views.sql |

Rules for whoever executes: follow CLAUDE.md (scope confirmation before code, one change at a time, npm run build must pass, no em dashes in copy). All items 1 to 9 are complete at the code level; deploy verification steps live in each row.
